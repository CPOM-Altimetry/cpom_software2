...
# Tool to plot altimetry waveforms
...
def update_map(
    ...
    Args:
        ...
        projection (ccrs.Projection): The Cartopy projection to use.
...
def get_default_names(
    ...
    Returns:
        tuple[str, str, str]: The waveform parameter name, latitude name, and longitude name.
    """
...
def get_variable(nc: Dataset, nc_var_path: str) -> Variable:
    ...
    for part in parts:
        try:
            ...
            sys.exit(f"NetCDF parameter '{nc_var_path}' not found")
    return var
...
def main(cmd_args: list[str]) -> None:
    ...
    """
    parser = argparse.ArgumentParser(description="Tool to plot altimetry waveforms")
    parser.add_argument(
    ...
    wf_name, latitude_name, longitude_name = get_default_names(
        file_name, args.parameter, args.latname, args.lonname
    )

    with Dataset(args.file) as nc:
        variable = get_variable(nc, wf_name)
        waveforms = variable[:].data

        lat_path = latitude_name.split("/")
        lon_path = longitude_name.split("/")
        latitudes = nc
        longitudes = nc
        for group in lat_path:
            latitudes = latitudes[group]
        for group in lon_path:
            longitudes = longitudes[group]
    ...
    button_height = 0.075
    button_padding = 0.02
    start_x = (1 - 5 * (button_width + button_padding) + button_padding) / 2
