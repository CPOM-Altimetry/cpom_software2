...
Alternative pass custom dataset definition file as dataset_filename argument to AltDataset class.
"""
...
class AltDataset:
    ...
    def __init__(
        ...
        Args:
            ...
            mission (str): Mission name (From supported missions in Missions.py)
            level (str): Data level, default is "l2".
            overrides (dict, optional): Keyword arguments to override default parameters.
            dataset_filename (str, optional): Path to custom dataset definition file.
                If provided, this file will be used instead of the default dataset definition.
        """
        self.name = name
        self.level = level
        self.mission = mission
        self._load_and_validate_dataset(overrides, dataset_filename)

    def _load_and_validate_dataset(
        self, overrides: dict | None = None, dataset_filename: str | None = None
    ):
        """Load dataset settings for current dataset name"""
        try:
            module = (
                importlib.import_module(dataset_filename)
            ...
            if self.name not in dataset_list:
                raise ValueError(f"{self.name} is not a supported dataset in AltDataset class")
            raise ImportError(f"Could not load dataset definition {self.name}") from exc

        # Set the dataset dictionary key/values as class attributes
        ...
        # Set derived parameters dictionary as a class attribute
        if hasattr(module, "derived_parameters") and isinstance(module.derived_parameters, dict):
            setattr(self, "derived_parameters", module.derived_parameters)

        # Set any attribute overrides passed at initialization
        if overrides:
    ...
    def get_files_dir(
        ...
        if "fdr4alt" in self.name:
            area_map = {"north": "greenland", "south": "antarctica"}
            area = area_map.get(hemisphere, "*")
    ...
    def get_files(
        ...
        def _conditional(file, yyyymm_str_fname_indices):
        ...
        if isinstance(max_dt_time, str):
            max_dt_time = datetime.strptime(max_dt_time, "%Y%m%d")

        search_dir = Path(self.get_files_dir(cyclenum, hemisphere, theme))
        if cyclenum is not None:
            return [file for file in Path(search_dir).rglob(self.search_pattern)]

        # Check if search directory has year/month structure
        first_month_dir = search_dir / f"{min_dt_time.year:04d}" / f"{min_dt_time.month:02d}"
        if first_month_dir.is_dir():
            files = []
            year_months = [
                (year, month)
                for year in range(min_dt_time.year, max_dt_time.year + 1)
                for month in range(1, 13)
                if (year > min_dt_time.year or month >= min_dt_time.month)
                and (year < max_dt_time.year or month <= max_dt_time.month)
            ]
            for year, month in year_months:
                month_dir = search_dir / f"{year:04d}" / f"{month:02d}"
                if month_dir.is_dir():
                    files.extend(
                        f
                        for f in month_dir.rglob(self.search_pattern)
                        if _conditional(f, self.yyyymm_str_fname_indices)
                    )
        else:
            files = [
                f
                for f in search_dir.rglob(self.search_pattern)
                if _conditional(f, self.yyyymm_str_fname_indices)
            ]
        return files
    ...
    def get_variable_from_file(self, nc: Dataset, nc_var_path: str) -> np.ndarray:
        """Retrieve variable from NetCDF file, handling groups if necessary.

        If the variable is a derived parameter, it will be computed using the provided function
        from the `derived_parameters` dictionary. Note that the derived parameter dictionary key
        must match the nc_var_paths exactly.
        ...
        """
        if self.derived_parameters and nc_var_path in self.derived_parameters:
            func = self.derived_parameters[nc_var_path]
            if not callable(func):
                raise TypeError(f"Derived parameter '{nc_var_path}' is not callable.")
            return func(lambda x: self.get_variable_from_file(nc, x), nc)
        ...
        if getattr(self, "data_packed_in_blocks", False):
            return var[:].flatten()
        return var[:]

    def get_variables_from_is2_file(self, nc: Dataset, nc_var_paths: str, beams: str) -> np.ndarray:
        """Retrieve variable(s) from Icesat-2 hdf5 file."""

        # TODO: Check if the beam is in the file.
        # Try except this code.
        if beams is None:
            beams = self.beams

        var_data = []
        for beam in beams:
            var = self.get_variable_from_file(nc, nc_var_paths.format(beam=beam))
            var_data.append(var)
