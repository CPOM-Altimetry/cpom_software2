...
def main():
            ...
            for i, param in enumerate(all_si_params):
                ...
                if "Fraction" not in param:
                    ...
                    try:
                        ...
                        print(f"Along track file not found: {alongtrack_file}")
                        continue
                    except pd.errors.EmptyDataError:
                        print(f"Along track file is empty: {alongtrack_file}")
                        continue
                ...
                for i, area in enumerate(area_plot_list):
                    ...
                    annotation_list.append(
                        Annotation(
                            0.685, 0.92, f"Latency: {latency}", fontsize=14, fontweight="normal"
                        )
                    )
    ...
    if args.season:
        ...
        for season in seasons:
            map_file = f"/cpnet/altimetry/seaice/{mission.upper()}/{archive_area}/archive/{season}{year}.map"
            ...
            for i, area in enumerate(area_plot_list):
                ...
                annotation_list.append(
                    Annotation(
                        0.32, 0.86, f"{thisarea.long_name}", fontsize=14, fontweight="normal"
                    )
                )
