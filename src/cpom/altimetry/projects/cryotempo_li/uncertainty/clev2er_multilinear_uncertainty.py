...
Script also assigns newly generated uncertainties to unseen elevations.
...
Linear model is fitted to smooth and fill the lookup table.
...
example usage: 
python clev2er_multilinear_uncertainty.py -a antarctica_icesheets -m median \
    -dh_create /media/luna/boxallk/clev2er/uncertainty_assessment/uncertainty_tables/antarctica_icesheets/SIN/2020 \
        -dh_assign /media/luna/boxallk/clev2er/uncertainty_assessment/uncertainty_tables/antarctica_icesheets/SIN/2022 \
...
def calculate_binned_metric(
    ...
    print(
        f"{sum(x is not None for x in [slope, roughness, power, coherence, poca_distance])} variables ingested"
    )

    # Create a DataFrame to hold the data
    data = pd.DataFrame(
        {"delta_elevation": np.abs(delta_elevation)}
    ...
    if poca_distance is not None:
        ...
        data["poca_distance_bin"] = pd.cut(
            data["poca_distance"],
            bins=poca_distance_bins,
            include_lowest=True,
            labels=poca_distance_bins[:-1],
        )
...
def get_binned_values(
    ...
    if len(ingested_variables) == 1:
        values = [
            binned_table_df.loc[ingested_variables[0][i]][0]
            for i in range(len(ingested_variables[0]))
        ]
    if len(ingested_variables) == 2:
        values = [
            binned_table_df.loc[ingested_variables[0][i]][ingested_variables[1][i]]
            for i in range(len(ingested_variables[0]))
        ]
    if len(ingested_variables) == 3:
        values = [
            binned_table_df.loc[ingested_variables[0][i]][ingested_variables[1][i]][
                ingested_variables[2][i]
            ]
            for i in range(len(ingested_variables[0]))
        ]
    ...
    if len(ingested_variables) == 5:
        values = [
            binned_table_df.loc[ingested_variables[0][i]][ingested_variables[1][i]][
                ingested_variables[2][i]
            ][ingested_variables[3][i]][ingested_variables[4][i]]
            for i in range(len(ingested_variables[0]))
        ]
...
def fit_linear_model(
    ...
    if len(ingested_variables) == 3:
        var1, var2, var3 = np.meshgrid(
            ingested_variables[0], ingested_variables[1], ingested_variables[2], indexing="ij"
        )
        var1 = var1.flatten()
        var2 = var2.flatten()
        var3 = var3.flatten()
    ...
    if len(ingested_variables) == 4:
        var1, var2, var3, var4 = np.meshgrid(
            ingested_variables[0],
            ingested_variables[1],
            ingested_variables[2],
            ingested_variables[3],
            indexing="ij",
        )
        var1 = var1.flatten()
        var2 = var2.flatten()
    ...
    if len(ingested_variables) == 2:
        ...
        # Apply the fit to the original grid
        linear_table = pd.DataFrame(
            linear_fit(var1, var2).reshape(binned_table.shape),
            index=binned_table.index,
            columns=binned_table.columns,
        )
    ...
    if len(ingested_variables) == 3:
        ...
        linear_table = pd.DataFrame(
            linear_fit(var1, var2, var3).reshape(binned_table.shape),
            index=binned_table.index,
            columns=binned_table.columns,
        )
    ...
    if len(ingested_variables) == 4:
        ...
        linear_table = pd.DataFrame(
            linear_fit(var1, var2, var3, var4).reshape(binned_table.shape),
            index=binned_table.index,
            columns=binned_table.columns,
        )
    ...
    if len(ingested_variables) == 5:
        ...
        x_matrix = np.vstack(
            [var1_clean, var2_clean, var3_clean, var4_clean, var5_clean, interaction_term]
        ).T
        reg = LinearRegression()
        reg.fit(x_matrix, z_clean)
        ...
        linear_table = pd.DataFrame(
            linear_fit(var1, var2, var3, var4, var5).reshape(binned_table.shape),
            index=binned_table.index,
            columns=binned_table.columns,
        )
...
def save_values_as_npz(values: list, filename: str) -> None:
    """Save values as a npz file.

    Args:
        values (list): Values to save.
        filename (str): The path to the file where the values will be saved.
    """
    np.savez(f"{filename}", values=values)


def plot_2d_slices(
    pivot_table, outpath: str, plot_var: str, ingested_vars: list, bins_1: list, bins_2: list
):
    """
    ...
    if len(ingested_vars) == 1:
        data = pd.pivot_table(
            pivot_table, index=[f"{ingested_vars[0]}_bin"], values=values, observed=False
        )
    ...
    # if more than one variable
        ...
        for c in combinations:
            ...
            data = pd.pivot_table(
                pivot_table,
                index=[f"{ingested_vars[indices[0]]}_bin"],
                columns=[f"{ingested_vars[indices[1]]}_bin"],
                values=values,
                observed=False,
            )
            _, ax = plt.subplots(figsize=(15, 6))
            ...
            filename = (
                outpath
                + f"2dslice_{plot_var}_{ingested_vars[indices[0]]}{ingested_vars[indices[1]]}.png"
            )
            plt.savefig(filename, bbox_inches="tight")
...
def main():
    ...
    parser.add_argument(
        "--area",
        "-a",
        choices=["antarctica_icesheets", "greenland_is"],
        help=("choose the ice sheet: 'antarctica_icesheets' (default) or 'greenland_is'"),
    )
    ...
    parser.add_argument(
        ...
        default="median",
        help=("choose the calculation method: 'median' (default) or 'mad' or 'rmse' or 'std_ue'"),
    )
    ...
    parser.add_argument(
        ...
        default="slope",
        help=(
            "choose the co-variates: 'slope' (default) or 'roughness' or 'power' or 'coherence' or 'poca_distance'"
        ),
    )
    ...
    parser.add_argument(
        ...
        "-s",
        default="cs2",
        help=("choose the satellite: 'cs2' (default) or 's3'"),
    )
    ...
    if args.area != "antarctica_icesheets" and args.area != "greenland_is":
        sys.exit("Must specify 'antarctica_icesheets' or 'greenland_is'")
    ...
    for path in glob.glob(f"{args.dh_dir_create}/**/*.npz", recursive=True):
        # for path in glob.glob(f'{args.dh_dir_assign}/S3*/{args.area}/2020/**/*.npz', recursive=True):
        ...
        if args.satellite == "cs2":
            if "power" in args.variables:
                # power = dh_data.get("pow")
                # power = dh_data.get("sig0_1_20_ku")
                power = dh_data.get("backscatter")
                power_all.extend(power)
            if "coherence" in args.variables:
                # coherence = dh_data.get("coh")
                # coherence = dh_data.get("coherence_20_ku")
                coherence = dh_data.get("coherence")
        ...
        elif args.satellite == "s3":
            if "power" in args.variables:
                power = dh_data.get("sig0_ice_sheet_20_ku")
                power_all.extend(power)
    ...
    if "slope" in args.variables:
        ...
        save_values_as_npz(the_slope_bins, args.outdir + f"/the_slope_bins_{args.area}.npz")
    if "roughness" in args.variables:
        print("Fetching roughness data")
        roughness = this_roughness.interp_roughness(
            lats_all, lons_all, method="linear", xy_is_latlon=True
        )
        ...
        save_values_as_npz(the_roughness_bins, args.outdir + f"/the_roughness_bins{args.area}.npz")
    if "power" in args.variables:
        ...
        save_values_as_npz(the_power_bins, args.outdir + f"/the_power_bins{args.area}.npz")
    if "coherence" in args.variables:
        ...
        save_values_as_npz(the_coherence_bins, args.outdir + f"/the_coherence_bins{args.area}.npz")
    if "poca_distance" in args.variables:
    ...
    save_values_as_pickle(
        binned_table_filled, args.outdir + f"/{args.area}_{args.method}_metric_pivot_filled.pickle"
    )

    # plot the plane fitted lookup table as 2D slices
    outpath = args.outdir + f"/{args.variables}_{args.method}_"
    plot_2d_slices(
        binned_table_filled, outpath, "look_up_table", variables, bins[0][:-1], bins[1][:-1]
    )
    ...
    if args.dh_dir_assign:
        ...
        for path in glob.glob(f"{args.dh_dir_assign}/**/*.npz", recursive=True):
            # for path in glob.glob(f'{args.dh_dir_assign}/S3*/{args.area}/2022/**/*.npz', recursive=True):
