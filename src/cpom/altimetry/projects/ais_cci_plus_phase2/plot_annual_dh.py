"""
cpom.altimetry.projects.ais_cci_plus_phase2.plot_annual_dh.py

# Purpose

Plot parameters from AIS CCI+ phase-2 annual dH products

Product naming:
ESACCI-AIS-L3C-SEC-MULTIMISSION_ANNUAL_CUMULATIVE_DH_GRID-5KM_199201_<YYYY><MM>-fv2.nc
where <MM> is normally 12 except for the last year where it could be 01..12
example:
ESACCI-AIS-L3C-SEC-MULTIMISSION_ANNUAL_CUMULATIVE_DH_GRID-5KM_199201_202212-fv2.nc
...
def main():
    ...
    parser.add_argument(
        "--prod_dir",
        "-d",
        help="path of input annual dH products",
    )

    parser.add_argument(
        "--prod_filename",
        "-f",
        help="path of input annual dH product file",
    )
    ...
    if args.prod_dir:
        input_files = glob.glob(
            f"{args.prod_dir}/ESACCI-AIS-L3C-SEC-MULTIMISSION_ANNUAL_CUMULATIVE_DH_GRID-5KM_*fv2.nc"
        )
    ...
    for input_file in input_files:
        ...
        dh_start_year = prod_name[62:66]
        dh_start_month = prod_name[66:68]
        dh_end_year = prod_name[69:73]
        dh_end_month = prod_name[73:75]
        print(f"dh_start_year={dh_start_year}")
        print(f"dh_start_month={dh_start_month}")
        print(f"dh_end_year={dh_end_year}")
        print(f"dh_end_month={dh_end_month}")
